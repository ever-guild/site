...
const MAIN_PARTICLES = 2600;
const HALO_PARTICLES = 1100;
const MIST_PARTICLES = 480;
...
const vertexShader = `
  ...
  void main() {
    ...
    // Gentle breathing scale; scroll nudges it a touch.
...
function createParticles(): ParticleData {
  const data: ParticleData = {
    ...
    scatter: new Float32Array(TOTAL_PARTICLES * 3),
...
export default function InfinityField({ reducedMotion = false, opacity = 1, scroll = 0 }: InfinityFieldProps) {
  ...
  if (w < 8) {
    scale = THREE.MathUtils.clamp(w / 15.5, 0.4, 0.52);
    verticalOffset = -0.35;
  } else if (w < 16) {
  ...
  useFrame((state, delta) => {
    ...
    if (materialRef.current) {
      ...
      currentScroll.current = THREE.MathUtils.damp(currentScroll.current, scrollRef.current, 4, delta);
      materialRef.current.uniforms.uScroll.value = currentScroll.current;
    }

    if (pointsRef.current) {
      pointsRef.current.rotation.z = Math.sin(t * 0.045) * 0.05;
      pointsRef.current.rotation.y = Math.sin(t * 0.06) * 0.1;
    }
  });
